...
export class PinCodeRegistrationRepository extends MemoryRepository<PinCodeRegistrationEntity> {
    ...
    async savePinCodeRegistration(registration: any): Promise<void> {
        ...
        if (existingRegistration) {
            ...
            this.logger.log(`Updated registration for PIN code: ${pinCode}, user: ${userId}`)
        } else {
            userRegistrations.push(registration);
            this.logger.log(`Created new registration for PIN code: ${pinCode}, user: ${userId}`)
        }
    }
    ...
    async findAll(): Promise<PinCodeRegistrationEntity[]> {
        ...
        this.logger.log(`Returning ${allRegistrations.length} registrations from the database.`);
    
        return allRegistrations;
    }

    async deletePicCodeRegistration(userId: string, pinCode: string): Promise<void> {
        const userRegistrations = this.database.get(userId)

        if(!userRegistrations) {
            throw new Error(`No registrations found for user: ${userId}`)
        }

        const index = userRegistrations.findIndex(reg => reg.pinCode === pinCode)
        if(index === -1) {
            throw new Error(`PIN code ${pinCode} not found for user: ${userId}`)
        }
    ...
    async validateRegistration(registration: any): Promise<void> {
        ...
        if(!userId || !pinCode || !doorsIds || !Array.isArray(doorsIds) || doorsIds.length === 0) {
            throw new Error("Invalid registration data. Ensure userId, pinCode, and doorIds are provided.")
        }
    }
    ...
    async isPinValidForDoor(pin: string, door: string, dataTime: Date): Promise<boolean> {
        ...
        for (const restriction of restrictions) {
            const validFrom = restriction.validFrom ? new Date(restriction.validFrom) : null;
            const validTo = restriction.validTo ? new Date(restriction.validTo) : null;
